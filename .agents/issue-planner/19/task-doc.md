...
- [ ] Add `site/src/components/ScoreboardRow.tsx` — props roughly
      `{ label, a, b, winner: 'a'|'b'|'tie', accentA, accentB, format?, tint?, valueA?, valueB? }`;
      renders the label, two opposing proportional bars (each fill = `value / max(a,b)`, `0` when
      `max===0`), both formatted values as text, and a winner glow in the winning player's accent
      (none on tie). Bar fill animates on entrance but renders final width immediately under reduced
      motion / jsdom (mirror `AnimatedNumber` in `components/AnimatedNumber.tsx`).
- [ ] Add `site/src/components/MetricScoreboard.tsx` — takes the `Comparison`, selects the headline
      ...
      `sharedGames`), renders a `SectionHeader title="Head to head"` + one `ScoreboardRow` each.
- [ ] Add `site/src/components/TrophyTiers.tsx` — selects `bronze`/`silver`/`gold`/`platinum` metrics
      by key, renders `SectionHeader title="Trophy tiers"` + one tier-tinted `ScoreboardRow` each
...
- [ ] Write component tests:
  - `site/src/components/ScoreboardRow.test.tsx` (winner-a glow, winner-b glow, tie neutral, values as text)
...
| `site/src/components/ScoreboardRow.tsx` | New — opposing-bars metric row with winner glow |
| `site/src/components/ScoreboardRow.test.tsx` | New |
